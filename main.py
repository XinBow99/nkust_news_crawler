...
class NKUST:
    ...
    def get_target_news_url(self, news_type, max_page=-1):
        ...
        last_page = int(re.findall(r'\d+', soup.find('span',
                        class_='pg-txt').text)[0]) if max_page == -1 else max_page
        total_news_url = []
    ...
    def get_target_news_content(self, news_url):
        ...
        ptinfoproperty_date = '' if ptinfoproperty_date is None else ptinfoproperty_date.find(
            'span').text.strip()
    ...
    def get_activity(self, max_item=-1):
        ...
        Snos = list(set(re.findall(r'Sno=(.*)"', requests.get(activity_url).text))
                    ) if max_item == -1 else Snos[:max_item]
        news_storage = []
        for Sno in Snos:
            _url = f"https://ws1.nkust.edu.tw/Activity/Home/Event?Sno={Sno}"
            result = requests.get(_url).text
            title = re.sub(r'&#\d+;|&\w+;', '',
                           re.findall(r'<h2>(.*)</h2>', result)[0])
            soup = BeautifulSoup(result, 'html.parser')
            content = re.sub(r'&#\d+;|&\w+;', '',
                             soup.find('div', class_='blog-main').text.strip())
            print(f"{title}")
            news_storage.append({
                'title': title,
                'url': _url,
    ...
    def get_about(self):
        ...
        for href in li_a_href:
            if href.startswith('/p/'):
                ...
                content = re.sub(
                    r'&#\d+;|&\w+;', '', _inner_soup.find('div', class_="mcont").text.strip())
                breadcrumb = re.sub(
                    r'&#\d+;|&\w+;', '', _inner_soup.find('ol', class_='breadcrumb').text.strip())
